...
void game::particles::advance_now()
{
    for (size_t i = 0; i < this->particles_async.size(); )
    {
        game::particles::particle_t& p = particles_async[i];
        if (p.delay > 0)
        {
            p.delay--;
            i++;
        ...
        else if (p.life > 1)
        {
            ...
            p.dist += p.dist_vel;
            p.spin += p.spin_vel;
            p.timer += ff::constants::seconds_per_update<float>();
            i++;
        }
        else
        {
...
void game::particles::release_group(uint16_t group_id)
{
    --this->groups[group_id].refs;
}

const DirectX::XMFLOAT4X4& game::particles::matrix(uint16_t group_id) const
{
...
bool game::particles::effect_active(int effect_id) const
{
...
void game::particles::effect_position(int effect_id, ff::point_fixed pos)
{
    for (auto& group : this->groups)
    {
        if (group.effect_id == effect_id && group.refs != -1)
...
ff::signal_sink<int>& game::particles::effect_done_sink()
{
...
game::particles::spec_t::spec_t(const ff::dict& dict)
{
...
size_t game::particles::spec_t::add(particles& particles, ff::point_fixed pos, int effect_id, const game::particle_effect_options& options) const
{
    size_t max_life = 0;
...
game::particles::effect_t::effect_t(const ff::value* value)
{
...
std::tuple<int, size_t> game::particles::effect_t::add(particles& particles, ff::point_fixed pos, const game::particle_effect_options* options) const
{
    return this->add(particles, &pos, 1, options);
}

std::tuple<int, size_t> game::particles::effect_t::add(particles& particles, const ff::point_fixed* pos, size_t pos_count, const game::particle_effect_options* options) const
{
    static std::atomic_int s_effect_id;
    int effect_id = s_effect_id.fetch_add(1) + 1;
    size_t max_life = 0;

    static game::particle_effect_options default_options;
